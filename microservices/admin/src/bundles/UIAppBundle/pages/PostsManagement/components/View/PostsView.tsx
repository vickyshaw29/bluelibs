...
import { PostViewer } from "../../config/PostViewer";
import { features } from "../../config/features";

export function PostsView(props: { id: string }) {
  ...
  if (isLoading) {
    ...
    if (error || document === null) {
      ...
      content = <PostsViewComponent document={document} />;
    }
  }
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.posts.view.header")}
...
export function PostsViewComponent(props: { document: Partial<Post> }) {
  const document = props.document;

  const viewer = use(PostViewer, { transient: true });
...
export function getHeaderActions(
  ...
  if (features.edit) {
    actions.push(
      <Link
        key="edit"
        to={router.path(Routes.POSTS_EDIT, {
          params: { id },
        })}
      >
      ...
      <Ant.Popconfirm
        ...
        onConfirm={() => {
          collection.deleteOne(id).then(() => {
            router.go(Routes.POSTS_LIST);
            Ant.notification.success({
              message: "Success",
              description: "You have deleted the Post",
            });
          });
        }}
      >
  ...
  const viewRoutePath = router.path(Routes.POSTS_VIEW, {
    params: { id },
  });
