...
import { PostsAntTableSmart } from "./PostsTableSmart";
...
export function PostsList() {
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.posts.list.header")}
        extra={[
          features.create ? (
            <Ant.Button
              key="1"
              onClick={() => router.go(Routes.POSTS_CREATE)}
              icon={<PlusOutlined />}
            >
              {t("management.posts.list.create_btn")}
            </Ant.Button>
          ) : null,
          <Ant.Button
            key="2"
      ...
      <Ant.Layout.Content>
        <Provider>
          <div className="page-posts-list">
            {filtersOpened && <PostsListFilters onUpdate={onFiltersUpdate} />}
            <Ant.Input.Search
              name="Search"
